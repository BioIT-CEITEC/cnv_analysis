...
for sample_name in tqdm(all_samples_names):
    ...
    sample_tmp_file = os.path.join(cur_dirname,"../tmp/") + sample_name + ".csv"
    ...
    if not os.path.exists(sample_tmp_file) or os.path.getsize(sample_tmp_file) == 0:
        message(f"No callable windows found for sample: {sample_name}")
        continue

    calls_ecole = pd.read_csv(sample_tmp_file, sep=",", header=None).values
...
if args.cnv == "merged":
    for sample_name in tqdm(all_samples_names):
        ...
        sample_tmp_file = os.path.join(cur_dirname,"../tmp2/") + sample_name + ".csv"
        ...
        if not os.path.exists(sample_tmp_file) or os.path.getsize(sample_tmp_file) == 0:
            message(f"No merged windows found for sample: {sample_name}")
            continue
...
shutil.rmtree(os.path.join(cur_dirname, "../tmp/"), ignore_errors=True)
shutil.rmtree(os.path.join(cur_dirname, "../tmp2/"), ignore_errors=True)
