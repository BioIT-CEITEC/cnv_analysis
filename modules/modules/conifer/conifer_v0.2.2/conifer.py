...
def CF_analyze(args):
	...
	for chr in chrs_to_process:
		...
		if num_chr_probes <= len(samples):
			print "[WARNING] Skipping %s: %d informative probes <= %d samples" % (cf.chrInt2Str(chr), num_chr_probes, len(samples))
			continue
...
def CF_bam2RPKM(args):
	...
	# pysam >=0.9 removed _hasIndex(); use has_index() if available
	if hasattr(f, "has_index"):
		_has_index = f.has_index()
	else:
		_has_index = f._hasIndex()
	if not _has_index:
