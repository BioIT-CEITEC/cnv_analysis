process CNVKIT_CLASSIFY_AND_ANNOTATE {
    ...
    input:
    tuple val(meta), path(cns_calls), path(preannot_bed), path(bed_calls), path(vcf_calls), path(preannot_tsv), emit: cnvkit_calls
    path annotation_tsv
    ...
    Rscript cnvAnnotateCNVkit.R ${preannot_tsv} classify/Scoresheet.txt ${annotation_tsv} annotate/${meta.donor}_final_CNVs_annotated.tsv annotate/${meta.donor}_final_CNVs_annotated.xlsx
    """
