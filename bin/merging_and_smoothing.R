...
parse_cnvkit <- function(file, max_size = NULL) {
  dt <- fread(file)
  if (nrow(dt) == 0) return(GRanges(tool=character(0),direction=integer(0),magnitude=numeric(0),confidence=numeric(0),cn_estimate=integer(0),qual_flag=integer(0),call_id=character(0)))
...
parse_gatk_gcnv <- function(file, max_size = NULL) {
  ...
  vcf <- tryCatch(
    ...
    error = function(e) {
      cat(sprintf("    WARNING: readVcf failed for %s: %s\n", basename(file), conditionMessage(e)))
      return(NULL)
    }
  )
  if (is.null(vcf) || length(vcf) == 0) {
    return(GRanges(
      tool = character(0), direction = integer(0),
      magnitude = numeric(0), confidence = numeric(0),
      cn_estimate = integer(0), qual_flag = integer(0),
...
load_cds_from_gtf <- function(gtf_file, capture_gr = NULL) {
  ...
  cds_raw[, gene_biotype := {
    attrs   <- attributes
    result  <- character(length(attrs))
    has_bio <- grepl('gene_biotype "', attrs, fixed = TRUE)
    if (any(has_bio))
      result[has_bio] <- gsub('.*gene_biotype "([^"]+)".*', "\\1",
                              attrs[has_bio], perl = TRUE)
    result
  }]
  ...
  gr <- GRanges(
    ...
    gene_biotype = cds_unique$gene_biotype
  )
  ...
  if (!is.null(capture_gr)) {
    ...
    dt_hits <- data.table(
      ...
      bed_label = capture_gr$bed_gene_name[subjectHits(hits)]
    )[, .(bed_gene_name = paste(unique(bed_label), collapse = ";")), by = exon_idx]
    gr <- gr[keep]
    bed_names <- dt_hits$bed_gene_name[match(keep, dt_hits$exon_idx)]
    bed_names[is.na(bed_names)] <- ""
    gr$bed_gene_name <- bed_names
    cat(sprintf("    %d CDS overlap with the regions of interest\n", length(gr)))
  } else {
    gr$bed_gene_name <- gr$gene_name
  }
...
build_per_exon_matrix <- function(tools_gr_list, exons_gr) {
    ...
    bed_gene_name = exons_gr$bed_gene_name,
    gene_biotype  = as.character(exons_gr$gene_biotype),
...
run_all <- function(input_dir,
  ...
  files <- list(
    ...
    gatk_gcnv    = if (!is.null(gatk_vcf_file)) {
      # Resolve relative paths against input_dir, just like every other tool
      if (!startsWith(gatk_vcf_file, "/") && !file.exists(gatk_vcf_file))
        file.path(input_dir, gatk_vcf_file)
      else
        gatk_vcf_file
    } else {
      file.path(input_dir, paste0(sample_id, "_gatk.vcf.gz"))
    }
  )
...
gatk_panelcnmops_agree <- function(dt) {
  has_gatk  <- all(c("gatk_gcnv_called",   "gatk_gcnv_direction")   %in% names(dt))
  has_panel <- all(c("panelcnmops_called", "panelcnmops_direction") %in% names(dt))
  if (!has_gatk || !has_panel) return(rep(FALSE, nrow(dt)))
  dt[["gatk_gcnv_called"]]   == 1L &
    dt[["panelcnmops_called"]] == 1L &
    dt[["gatk_gcnv_direction"]] != 0L &
    dt[["gatk_gcnv_direction"]] == dt[["panelcnmops_direction"]]
}
...
make_merged_target_consensus <- function(per_exon_dt, tool_names, sample_id,
  ...
  dt <- per_exon_dt[(n_tools_called >= min_callers | gatk_panelcnmops_agree(per_exon_dt)) & end > start]
  if (nrow(dt) == 0) {
...
make_smoothed_variants <- function(per_exon_dt, sample_id, out_file,
  ...
  dt <- per_exon_dt[(n_tools_called >= min_callers | gatk_panelcnmops_agree(per_exon_dt)) &
  ...
  get_cn_for_row <- function(row) {
    result <- list()
    for (t in active_tools) {
      col_c  <- paste0(t, "_called")
      col_cn <- paste0(t, "_cn_estimate")
      if (col_c %in% names(row) && isTRUE(row[[col_c]] == 1L) &&
          col_cn %in% names(row) && !is.na(row[[col_cn]])) {
        result[[t]] <- as.integer(row[[col_cn]])
      }
    }
    result
  }

  merge_cn <- function(base, new_cn) {
    for (t in names(new_cn)) base[[t]] <- c(base[[t]], new_cn[[t]])
    base
  }

  build_cn_label <- function(cn_per_tool) {
    parts <- character(0)
    for (t in active_tools) {
      vals <- cn_per_tool[[t]]
      if (length(vals) > 0)
        parts <- c(parts, paste0(t, ":", as.integer(round(median(vals)))))
    }
  ...
  for (i in seq_len(nrow(dt))) {
    ...
    if (!is.null(current) &&
      ...
      current$BED_gene_name  <- paste(unique(c(
        strsplit(current$BED_gene_name, ";")[[1]],
        as.character(row$bed_gene_name)
      )), collapse = ";")
      current$gene_biotype   <- paste(unique(c(
        strsplit(current$gene_biotype, ";")[[1]],
        as.character(row$gene_biotype)
      )), collapse = ";")
      current$cn_per_tool    <- merge_cn(current$cn_per_tool, cn_i)
    } else {
      ...
      current <- list(chr = as.character(row$chr), start = as.integer(row$start),
                      ...
                      BED_gene_name = as.character(row$bed_gene_name),
                      gene_biotype  = as.character(row$gene_biotype),
                      cn_per_tool = cn_i)
    }
  }
  if (!is.null(current)) cnvs <- c(cnvs, list(current))
  ...
  out <- rbindlist(lapply(cnvs, function(x) data.table(
    ...
    BED_gene_name = x$BED_gene_name,
    gene_biotype  = x$gene_biotype,
    cn_label = build_cn_label(x$cn_per_tool)
  )))
  fwrite(out, out_file, sep = "\t")
  if (!is.null(bed_file))
    fwrite(out[, .(CHR = CHROM, START, END, TYPE = consensus_type)], bed_file, sep = "\t", col.names = FALSE)
